    ...
    <header class="bg-white shadow-sm border-b border-slate-200 px-4 lg:px-8 py-4">
     <div class="flex items-center justify-between">
      <div class="flex items-center gap-4"><!-- Mobile Menu Button --> <button onclick="toggleSidebar()" class="lg:hidden p-2 rounded-lg hover:bg-slate-100 transition-colors">
       ...
       <div>
        <h2 id="pageTitle" class="text-xl font-bold text-slate-800">Data Sekolah</h2>
       </div>
      </div>
     </div>
    </header>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-200">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h3 class="text-lg font-bold text-slate-800">Data Sekolah</h3>
              <p class="text-sm text-slate-500">Kelola data sekolah</p>
            </div>
            <button onclick="openModal('sekolahModal')"
          ...
          <table id="navTable"
            ...
            <thead class="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th class="px-4 py-3 text-left font-semibold text-slate-700 border-r border-slate-200">NPSN</th>
                <th class="px-4 py-3 text-left font-semibold text-slate-700 border-r border-slate-200">Nama Sekolah</th>
                <th class="px-4 py-3 text-left font-semibold text-slate-700 border-r border-slate-200">Alamat</th>
            ...
            $no=1;
            $query=mysqli_query($conn,"SELECT * FROM tb_sekolah_asal");
            while($row=mysqli_fetch_assoc($query)){
            ?>
              <tr class="hover:bg-slate-50 transition-colors">
                <td class="px-4 py-3 border-r border-slate-100"><?php echo $no++;?></td>
                <td class="td-npsn px-4 py-3 border-r border-slate-100"><?php echo $row['npsn']?></td>
                <td class="td-namasekolah px-4 py-3 border-r border-slate-100"><?php echo $row['nama_sekolah']?></td>
                <td class="td-alamat px-4 py-3 border-r border-slate-100"><?php echo $row['alamat']?></td>
                ...
                data-id="<?php echo $row['sekolah_id']; ?>"
                title="Edit Sekolah"
              ...
              <!-- Tombol Hapus -->
              <a href="sekolah/hapus.php?id=<?php echo $row['sekolah_id']; ?>"
