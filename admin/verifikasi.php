    ...
    <header class="bg-white shadow-sm border-b border-slate-200 px-4 lg:px-8 py-4">
     <div class="flex items-center justify-between">
      <div class="flex items-center gap-4"><!-- Mobile Menu Button --> <button onclick="toggleSidebar()" class="lg:hidden p-2 rounded-lg hover:bg-slate-100 transition-colors">
       ...
       <div>
        <h2 id="pageTitle" class="text-xl font-bold text-slate-800">Verifikasi Data</h2>
       </div>
      </div>
     </div>
    </header>
    ...
    <div class="p-6 border-b border-slate-200">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-slate-500">Lakukan verifikasi agar pendaftaran selesai</p>
        </div>
      </div>
    </div>
        ...
        <table id="verifikasiTable"
          ...
          <thead class="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th class="px-4 py-3 text-left font-semibold text-slate-700 border-r border-slate-200">Tgl Submit</th>
          ...
          $query = mysqli_query($conn, "SELECT 
                  ...
                  INNER JOIN tb_sekolah_asal s
                      ON p.sekolah_id = s.sekolah_id WHERE p.status='submit'
          ");
          ...
          ?>
            <tr class="hover:bg-slate-50 transition-colors">
              ...
              <td class="td-telp px-4 py-3 border-r border-slate-100"><?php echo $row['tanggal_submit']?></td>
              <td class="px-4 py-3 text-center bg-white sticky right-0 z-10 border-l border-slate-200">
               <!-- Tombol Verifikasi -->
                <form action="verifikasi/simpan.php" method="POST" class="inline">
                <input type="hidden" name="pendaftaran_id"
                      value="<?php echo $row['pendaftaran_id']; ?>">

                <button type="submit"
                  onclick="return confirm('Verifikasi data pendaftar ini?')"
